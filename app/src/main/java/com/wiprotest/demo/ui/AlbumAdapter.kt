...
class AlbumAdapter(val results: Results, val screenSizeName: String) :
    ...
    fun bind(results: Results, screenSizeName: String) {
        if (screenSizeName.equals("small")) {
            Glide.with(itemView.context)
                .load(results.albummatches.album[layoutPosition].image.get(0).text).into(photoIV)
        } else if (screenSizeName.equals("medium")) {
            Glide.with(itemView.context)
                .load(results.albummatches.album[layoutPosition].image.get(1).text).into(photoIV)
        } else if (screenSizeName.equals("large")) {
            Glide.with(itemView.context)
                .load(results.albummatches.album[layoutPosition].image.get(2).text).into(photoIV)
        } else if (screenSizeName.equals("extralarge")) {
            Glide.with(itemView.context)
                .load(results.albummatches.album[layoutPosition].image.get(3).text).into(photoIV)
        }

        artistTV.text = results.albummatches.album[layoutPosition].artist
